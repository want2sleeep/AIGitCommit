    ...
    describe('Successful Flow', () => {
        it('should complete full commit message generation flow', async () => {
            ...
            const mockShowInputBox = jest.spyOn(vscode.window, 'showInputBox')
                .mockResolvedValue('feat(test): add goodbye function and update hello message\n\n- Modified hello function to print "Hello World"\n- Added new goodbye function');

            const mockWithProgress = jest.spyOn(vscode.window, 'withProgress')
                .mockImplementation(async (_options, task) => {
                    return task({ report: jest.fn() }, { isCancellationRequested: false } as vscode.CancellationToken);
                });
