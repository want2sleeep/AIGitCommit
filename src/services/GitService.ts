...
import { GitChange, ChangeStatus, GitAPI, GitExtension, GitRepository, GitFileChange } from '../types';
...
export class GitService {
    ...
    private initializeGitExtension(): void {
        ...
        if (this.gitExtension) {
            if (!this.gitExtension.isActive) {
                this.gitExtension.activate().then(gitApi => {
     ...
     */
    private async convertToGitChange(change: GitFileChange, repository: GitRepository): Promise<GitChange | null> {
