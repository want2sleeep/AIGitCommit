...
[![Version](https://img.shields.io/badge/Version-1.4.4-orange.svg)](package.json)
...
# 输入并执行以下任一命令：
"AI Git Commit: Configure Settings"
"AI Git Commit: 配置AI Git Commit"
"配置 AI Git Commit"
```

### 2. 选择提供商并配置

在配置面板中选择您的 AI 服务提供商并输入相关信息：

| 提供商 | 推荐模型 | 特点 |
|--------|---------|------|
| OpenAI | gpt-3.5-turbo / gpt-4 | 质量高，速度快 |
| Google Gemini | gemini-1.5-flash | 快速实惠 |
| Qwen | qwen-turbo | 国内访问快 |
| Ollama | codellama | 本地运行，隐私保护 |
| vLLM | Llama-2 | 高性能本地推理 |
...
混合模型策略是一个智能的成本优化功能，采用"快模型读(Map)，慢模型写(Reduce)"的理念。在处理大型提交时，使用轻量级快速模型处理大量 chunk（Map 阶段），使用高质量模型生成最终提交信息（Reduce 阶段）。这样可以：

- **大幅降低成本**: Map 阶段使用轻量级模型，成本降低 85%+
- **显著提升速度**: 轻量级模型响应更快，处理时间减少 40%+
- **保证输出质量**: Reduce 阶段使用高质量模型，确保最终提交信息质量
- **智能自动降级**: 根据主模型自动选择合适的轻量级模型

#### 工作原理

```mermaid
graph LR
    A[大型提交] --> B[分割为多个 Chunks]
    B --> C[Map 阶段: 轻量级模型]
    C --> D[生成 Chunk 摘要]
    D --> E[Reduce 阶段: 高质量模型]
    E --> F[生成最终提交信息]
...
**场景**: 处理包含 20 个 chunks 的大型提交

| 策略 | Map 阶段 | Reduce 阶段 | 总成本 | 总时间 | 成本节省 | 时间节省 |
|------|---------|------------|--------|--------|---------|---------|
| **传统方式** | 20 × GPT-4 | 1 × GPT-4 | 21x | 10s | - | - |
| **混合策略** | 20 × gpt-4o-mini | 1 × GPT-4 | 3x | 6s | **85.7%** | **40%** |
...
系统会根据您配置的主模型自动选择合适的轻量级模型：

| 主模型 | 自动降级为 | 成本比例 | 速度提升 |
|--------|-----------|---------|---------|
| GPT-4 系列 | gpt-4o-mini | 10% | 2x |
| Gemini Pro | gemini-1.5-flash | 5% | 3x |
| 其他模型 | 保持原模型 | - | - |
| 本地模型 | 不降级 | 免费 | - |

**注意**: 本地模型（Ollama、LM Studio）不会进行降级，因为本地运行不产生费用。

#### 配置选项

```json
{
  // 启用 Map-Reduce 处理（大型提交自动启用）
  "aigitcommit.enableMapReduce": true,

  // 指定 Map 阶段使用的轻量级模型（留空则自动降级）
...
```json
{
  ...
  "aigitcommit.chunkModel": ""  // 留空，自动降级为 gpt-4o-mini
}
```
...
```json
{
  "aigitcommit.provider": "openai",
  "aigitcommit.modelName": "gpt-4",
  "aigitcommit.chunkModel": "gpt-4o-mini"  // 手动指定
}
```
...
```json
{
  ...
  "aigitcommit.chunkModel": ""  // 本地模型不降级
}
```

#### 推荐的轻量级模型

| 模型 | 提供商 | 相对成本 | 相对速度 | 适用场景 |
|------|--------|---------|---------|---------|
| gpt-4o-mini | OpenAI | 10% | 2x | Map 阶段、快速处理 |
| gemini-1.5-flash | Google | 5% | 3x | Map 阶段、超快速处理 |
| gpt-3.5-turbo | OpenAI | 5% | 2.5x | 预算优先 |
| qwen-turbo | Qwen | 3% | 2x | 国内用户 |
...
- ✅ **频繁提交**: 需要经常生成提交信息
- ✅ **成本敏感**: 希望降低 API 调用成本
- ✅ **云端模型**: 使用 OpenAI、Gemini 等付费服务
- ❌ **小型提交**: 少于 10 个文件（自动跳过 Map-Reduce）
- ❌ **本地模型**: 使用 Ollama 等本地模型（不产生费用）
...
| `aigitcommit.enableMapReduce` | `true` | 启用 Map-Reduce 处理大型提交 |
| `aigitcommit.chunkModel` | `""` | Map 阶段使用的轻量级模型（留空则自动降级） |
